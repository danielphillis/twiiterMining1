...
search_results = []
v = 10 #optional version identifier suffix for filename
...
for q in queries:
	...
	size = 25
	fd = analyse_content(search_results, keywords, size) #prints a prettyTable
	#returns a dictionary for writing a FREQ DIST	

	# add Timestamp to filename
	# timestamp = ("{:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now())
	#print str(timestamp)

	q = q + path + str(v)# append output path and version number to query text

	#writeCSVData(q,words) # append ?
	writeCSVDict(q, fd) #file gets the query name
